...
Часть функций доступна только администраторам. Кто админ — задаёт `ADMIN_ID`
в `.env`: один ID или **несколько через запятую** (`ADMIN_ID=787751346,1268132424`).
Пустое значение отключает админ-функции. Сообщения не-админов до этих команд не
доходят (фильтр `IsAdmin`), как будто команд нет.

- **`/top [предмет]`** — топ учеников по предмету (числовые баллы по убыванию,
  «Зачёт» — в конце; баллы показываются открыто). Без аргумента — список предметов,
  по которым есть результаты, с числом учеников. Предмет можно писать как на сайте
  (`/top математика профильная`) — синонимы сводятся к одному ключу.
- **`/check`** — вручную запускает полную проверку всех учеников в фоне (как цикл по
  расписанию): рассылает найденные баллы подписчикам и присылает админам сводку.
