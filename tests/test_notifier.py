...
class FakeBot:
    ...
    def __init__(self, fail_ids: set[int] | None = None, retry_ids: set[int] | None = None):
    ...
    async def send_message(self, telegram_id: int, text: str) -> None:
        ...
        if telegram_id in self._retry_pending:
            ...
            raise TelegramRetryAfter(method=None, message="slow down", retry_after=0)
        self.sent.append((telegram_id, text))


async def test_broadcast_sends_to_all():
    bot = FakeBot()
    notifier = Notifier(bot, broadcast_delay=0)
...
async def test_broadcast_throttles_between_messages(monkeypatch):
    ...
    bot = FakeBot()
    notifier = Notifier(bot, broadcast_delay=0.01)
