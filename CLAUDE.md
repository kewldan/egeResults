...
- **Постоянная нижняя клавиатура** (`keyboards.main_reply_keyboard`, `is_persistent`): «📋 Мои ученики» (алиас `/start`), «🛡 Безопасность», «ℹ️ О боте» — ловятся в `common` по `F.text == texts.BTN_*` (подписи кнопок — единый источник правды в `texts.py`). Ставится при `/start` и при приёме приглашения (только отправка сообщения умеет задать reply-клавиатуру, поэтому `/start` шлёт приветствие с ней + отдельным сообщением инлайн-меню).
