...
2. Для каждого: `provider.fetch(StudentQuery)` → `StudentSnapshot` (`results: list[FetchedResult]` + `registrations` — регистрации на экзамены, student-уровень).
...
**Чтобы добавить источник:** реализовать `fetch()` → `StudentSnapshot` (бросать `StudentNotFoundError`, когда ученик не найден; `registrations=[]`, если их нет), добавить значение в `Literal` в `config.Settings.provider` и ветку в `build_provider()`.
...
- HTTP вынесен из логики: `build_form_body()`, `parse_results_html()`, `parse_registrations()` и `looks_not_found()` — чистые функции (покрыты тестами на санитизированном HTML и точном теле запроса в `tests/fixtures/`). `looks_not_found()` отличает страницу-форму поиска (ученик не найден) от страницы найденного ученика по наличию блока `#exam-content`/`#result-data`/`#reg-data`; `fetch()` на форму поиска бросает `StudentNotFoundError`.
- Результат бывает числом (балл → `score`) или текстом («Зачёт» → `value`); диф сравнивает `value`/`score`/`status`.
- **Детальный парсинг** (см. отдельный раздел ниже): `parse_results_html(html, base_url=...)` помимо баллов собирает критерии, первичный балл, распознанные ответы и сканы бланков по каждому экзамену; `parse_registrations()` — таблицу «Регистрация на экзамены» (`#reg-data`). Эта детализация в diff НЕ участвует — её появление не вызывает повторных уведомлений.
...
- **Детализация результата** (встроена в `Student`, в diff не участвует): `ResultItem` помимо `score`/`value`/`status` несёт `criteria` (`Criterion`), `primary_score`, `recognition` (`TaskAnswer`), `blanks` (`BlankImage` с `url` + локальным `path`); `Student.registrations` (`Registration`: дата/предмет/ППЭ/адрес) — отдельно от `results`, т.к. регистрация бывает на экзамен без результата.
...
- Сервисы инжектятся в хендлеры через workflow data: в `factory.build_dispatcher` делается `dp["subscriptions"|"results"|"notifier"|"settings"|"cards"|"blanks"] = ...`, а хендлеры получают их **по имени аргумента** (`subscriptions`, `results`, `notifier`, `settings`, `cards`, `blanks`). Имя параметра обязано совпадать с ключом (`cards`/`blanks` бывают `None`, если выключены).
...
### Детальный парсинг страницы результата и бланки (`providers/ege_spb.py`, `services/blanks.py`)

Страница результата ученика (`#result-data`) несёт не только баллы, но и детализацию по каждому экзамену; парсеры чистые (фикстура `tests/fixtures/ege_spb_sample.html` — **санитизированная**: фейковые токены бланков и фейковые ответы, живые в репо не коммитим).

- `parse_results_html(html, base_url=...)` собирает в `FetchedResult` (а через `merge_results` — в `ResultItem`): критерии («Результаты по критериям» и сводку «Первичные баллы по частям» → `criteria`), `primary_score` («Первичный балл (сумма)»), распознанные ответы по заданиям (`recognition`), сканы бланков (`blanks` — ссылки `download.php`, абсолютизируются `base_url`). Парсинг идёт по блокам `#exam-detail` (на старых/тестовых страницах без обёртки — по `.exam-title`); извлечение предмета/балла/статуса оставлено байт-в-байт прежним (от него зависит diff).
- `parse_registrations(html)` — таблица «Регистрация на экзамены» (`#reg-data`): дата, предмет, ППЭ + адрес. Student-уровень (`Student.registrations`), в diff не участвует.

**Никаких повторных уведомлений после расширения парсера** — ключевой инвариант: `diff._is_changed` сравнивает ТОЛЬКО `value`/`score`/`status`; детализация (критерии/баллы/распознавание/бланки/регистрации) туда НЕ входит специально. `merge_results` сохраняет деталь свежую, иначе прежнюю (`_detail_kwargs`), но `updated_at` двигает лишь при изменении value/score/status. Регрессии закреплены в `tests/test_diff.py`.

**Показ в карточке:** `student_card_keyboard(student, *, with_card=...)` принимает сам объект `Student` и показывает кнопки 📅 Расписание / 📊 Детали / 📄 Бланки **условно** (есть регистрации / есть критерии-баллы-ответы / есть бланки). Хендлеры `schedule:`/`details:`/`blanks:` (`my_students.py`) через общий `_authorized_student` (доступ только подписчику). Расписание/детали правят сообщение (`back_to_card_keyboard`); критерии/баллы тут показываются **открыто, без спойлера** (владелец сам смотрит свою детализацию).

**Бланки на диск** (`services/blanks.py::BlankDownloader`, инжектится `dp["blanks"]` и в `ResultsService`): при проверке `ResultsService._download_blanks` качает новые сканы в `BLANKS_DIR/<identity_hash>/<Предмет>__<Лист>.<ext>` (ключ — `identity_hash`, не `id`: стабилен и без PII), уже скачанные не перекачивает (ищет файл по имени без расширения — `blank_stem`), пишет путь в `BlankImage.path`. Кнопка отдаёт файл **с диска** (`FSInputFile`), иначе докачивает по ссылке — так бланки переживают протухание одноразовых `download.php`. **Бланки одного предмета уходят одним сообщением-альбомом** (`answer_media_group`, до 10 листов; единственный лист — `answer_document`), у разных предметов — разными сообщениями (`_send_subject_blanks` группирует `student.results` по `subject_title`). Отправка устойчива (`_send_with_retry`): сбой альбома по одному предмету не обрывает остальные. Best-effort: сбой скачивания не валит проверку. Конфиг `DOWNLOAD_BLANKS`/`BLANKS_DIR`; в docker — named-volume `blanks` на `/app/data/blanks` (бот под non-root uid, поэтому **named**, а не bind; каталог создаётся в образе под `appuser`, чтобы volume унаследовал владельца).
