...
# Алерты callback.answer — без HTML (Telegram показывает их как простой текст).
CARD_NO_RESULTS = "Результатов пока нет — картинку соберу, как только появятся баллы."
CARD_FAILED = "Не удалось собрать картинку. Попробуйте чуть позже."


def card_caption(student: Student) -> str:
    """Подпись под картинкой результатов — с приглашением выложить в сторис."""
    return (
        f"🎉 Готова карточка с результатами: <b>{_esc(student.last_name)}</b>!\n\n"
        "Сохрани картинку и выложи в сторис — пусть все увидят твои баллы 🔥"
    )
