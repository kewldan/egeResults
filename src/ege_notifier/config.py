...
class Settings(BaseSettings):
    ...
    # --- Сканы бланков ответов ---
    # При проверке бот скачивает сканы бланков (ссылки download.php) и кладёт их в
    # BLANKS_DIR (data/blanks/<identity_hash>/...). Так бланки переживают протухание
    # одноразовых ссылок и не качаются заново при каждом открытии. В docker — смонтируй
    # сюда volume, иначе файлы пропадут при пересоздании контейнера (бот их докачает).
    # DOWNLOAD_BLANKS=false — не качать на диск (кнопка тогда качает «на лету»).
    download_blanks: bool = True
    blanks_dir: str = "data/blanks"
